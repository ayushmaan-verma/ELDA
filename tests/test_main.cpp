...
void test_precision_row_scaling() {
    ...
    assert(is_close(I_check(0, 0), 1.0, 1e-6));
    assert(is_close(I_check(1, 1), 1.0, 1e-6));
    assert(is_close(I_check(0, 1), 0.0, 1e-6));
    assert(is_close(I_check(1, 0), 0.0, 1e-6));
...
void test_tolerant_comparisons() {
    std::cout << "Running tolerant comparison tests..." << std::endl;

    // 1. Rotation matrix test (Trigonometric functions create small perturbations)
    double theta = linalg::PI / 4.0;
    ...
    assert(linalg::check_unitary(R));

    // 2. Perturbation test: two matrices differ by a tiny amount (within tolerance)
    linalg::matrix A(2, 2);
    A(0, 0) = 1.0; A(0, 1) = 2.0;
    A(1, 0) = 3.0; A(1, 1) = 4.0;

    linalg::matrix B = A;
    B(0, 1) += 1e-7; // very small perturbation, below EPS (1e-6)

    assert(A == B);

    // If perturbation is larger than EPS, they should NOT be equal
    B(0, 1) += 1e-5;
    assert(!(A == B));

    // 3. Shape mismatch test (should return false and not crash or throw out_of_range)
    linalg::matrix C(1, 4);
    C(0, 0) = 1.0; C(0, 1) = 2.0; C(0, 2) = 3.0; C(0, 3) = 4.0;
    assert(!(A == C));

    // 4. Triangular check tolerance
    linalg::matrix U(2, 2);
    U(0, 0) = 1.0; U(0, 1) = 2.0;
    U(1, 0) = 1e-8; // tiny sub-diagonal element (below EPS)
    U(1, 1) = 3.0;
    assert(U.check_upper_tri());

    linalg::matrix L(2, 2);
    L(0, 0) = 1.0; L(0, 1) = 1e-8; // tiny super-diagonal element
    L(1, 0) = 2.0; L(1, 1) = 3.0;
    assert(L.check_lower_tri());

    std::cout << "Tolerant comparison tests passed!" << std::endl;
}
