...
void test_is_square_property() {
    std::cout << "Running is_square property tests..." << std::endl;

    linalg::matrix square_mat(3, 3);
    assert(square_mat.is_square() == true && "3x3 matrix should evaluate as square.");

    linalg::matrix rect_mat(4, 2);
    assert(rect_mat.is_square() == false && "4x2 matrix should evaluate as rectangular.");

    linalg::matrix flat_mat(1, 5);
    assert(flat_mat.is_square() == false && "1x5 matrix should evaluate as rectangular.");

    std::cout << "is_square property tests passed!" << std::endl;
}
