...
/// Helper function to perform approximate comparison between two floating-point values.
inline bool close_enough(double a, double b, double tol = EPS) {
...
class matrix {
  public:
    ...
    matrix(int r, int c, double val) : row(r), col(c), arr(r * c, val) {
        if (r < 0 || c < 0) {
            throw std::runtime_error("Matrix dimensions must be non-negative.");
        }
    }
